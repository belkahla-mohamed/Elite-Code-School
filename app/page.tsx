      ...
      <section id="how-it-works" className="bg-[#F4F8FC] py-20">
        ...
        <div className="container-shell grid gap-5 md:grid-cols-3">
          {[
            ...
            <article key={String(title)} className="rounded-brand border-2 border-[#E8EEF6] bg-white p-8 text-center">
              {typeof Icon !== "string" && <Icon className="mx-auto size-12 text-sky" />}
              <h3 className="mt-5 font-display text-2xl font-black">{String(title)}</h3>
              <p className="mt-3 text-sm font-semibold leading-6 text-ink-soft">{String(text)}</p>
            </article>
          ))}
        </div>
      </section>
  ...
  return (
    <header className="fixed inset-x-0 top-0 z-50 border-b-2 border-[#E8EEF6] bg-white">
      <div className="container-shell flex items-center justify-between py-4">
        <Link href="/" className="flex items-center gap-3 font-display text-2xl font-black tracking-[-0.04em]">
          <span className="flex size-12 items-center justify-center rounded-2xl bg-sky text-white">
            <Code2 />
          </span>
          <span>
            Elite Code
            <span className="block -mt-2 text-base text-sky">School</span>
          </span>
        </Link>
        <nav className="hidden rounded-full bg-sky px-6 py-3 text-xs font-black uppercase tracking-wide text-white md:flex md:items-center md:gap-6">
          <a href="#formations">Why Elite</a>
          <a href="#portfolios">For parents</a>
          <Link href="/teacher">For teachers</Link>
        </nav>
        <div className="flex items-center gap-3">
          <Link href="/login" className="hidden rounded-full border-2 border-pink px-5 py-2 text-xs font-black uppercase tracking-wide text-pink sm:inline-flex">
            Log in
          </Link>
  ...
  return (
    <footer id="contact" className="bg-ink py-12 text-white">
      <div className="container-shell grid gap-8 md:grid-cols-[2fr_1fr_1fr]">
        <div>
          <div className="font-display text-3xl font-black">Elite Code School</div>
          <p className="mt-3 max-w-md text-white/60">
            Une academy STEM lumineuse pour apprendre la robotique, le code et l'intelligence artificielle.
          </p>
        </div>
        <div>
          <h3 className="font-mono text-sm uppercase tracking-wider text-white/50">Accès</h3>
        ...
        <div>
          <h3 className="font-mono text-sm uppercase tracking-wider text-white/50">Contact</h3>
          <p className="mt-4 text-white/70">Marrakech · Maroc<br />contact@elitecodeschool.ma</p>
        </div>
      </div>
    </footer>
  );
