  ...
  return (
    <header className="sticky top-0 z-30 flex h-16 items-center justify-between border-b-2 border-border bg-surface px-4 md:px-6">
      <div className="flex min-w-0 items-center gap-3">
        ...
        <h1 className="min-w-0 truncate font-display text-lg font-black text-ink sm:text-xl">{title}</h1>
      </div>
        ...
        <DropdownMenu>
          ...
          <DropdownMenuContent align="end" className="w-48">
            ...
            <DropdownMenuItem onClick={() => router.push("/dashboard/settings")}>
              <Gear className="mr-2 size-4" /> Paramètres
            </DropdownMenuItem>
            ...
            <DropdownMenuItem onClick={handleLogout} className="text-coral">
              <SignOut className="mr-2 size-4" /> Déconnexion
            </DropdownMenuItem>
          </DropdownMenuContent>
        </DropdownMenu>
      </div>
